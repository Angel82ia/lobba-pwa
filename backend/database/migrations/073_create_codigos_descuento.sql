...
CREATE TABLE IF NOT EXISTS codigos_descuento (
  ...
  porcentaje_comision DECIMAL(5,2) NOT NULL DEFAULT 10.00, -- Comisión para influencer (FIJO 10%)
  ...
  CONSTRAINT comision_valida CHECK (porcentaje_comision = 10.00)
);
...
SELECT 'MARIA10', id, 10.00, 10.00 
FROM codigos_influencers 
WHERE codigo = 'MARIA2024'
ON CONFLICT (codigo) DO NOTHING;

INSERT INTO codigos_descuento (codigo, influencer_id, porcentaje_descuento, porcentaje_comision)
SELECT 'TEST10', id, 10.00, 10.00 
FROM codigos_influencers 
...
COMMENT ON COLUMN codigos_descuento.porcentaje_comision IS 'Porcentaje de comisión para el influencer SIEMPRE FIJO en 10% del importe original';
